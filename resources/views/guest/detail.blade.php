    ...
    <div class="mt-4">
        <div class="row justify-content-between">
            ...
            <div class="col-md-8">
                ...
                @php
                    // Memeriksa apakah ada ulasan pengguna untuk buku ini
                    $ulasanPengguna = $ulasan->where('user_id', auth()->id())->first();
                @endphp
                    ...
                    @elseif($statusPeminjaman === 'dipinjam')
                        ...
                        <form action="{{ route('ulasan.store', ['id' => $buku->id]) }}" method="post">
                            ...
                            <div class="form-group mt-3">
                                ...
                                <textarea name="ulasan" id="ulasan" class="form-control" required {{ $statusPeminjaman === 'dipinjam' && !$ulasanPengguna ? '' : 'disabled' }}>{{ $ulasanPengguna ? $ulasanPengguna->ulasan : '' }}</textarea>
                            </div>
                            <div class="form-group mt-3">
                                <label for="rating">Rating (1-5):</label>
                                <input type="number" name="rating" id="rating" class="form-control" min="1" max="5" required {{ $statusPeminjaman === 'dipinjam' && !$ulasanPengguna ? '' : 'disabled' }} value="{{ $ulasanPengguna ? $ulasanPengguna->rating : '' }}">
                            </div>
                            <br>
                            <button type="submit" class="btn btn-primary" {{ ($statusPeminjaman === 'dipinjam' && !$ulasanPengguna) || $statusPeminjaman === 'dikembalikan' ? '' : 'disabled' }}>
                                @if($ulasanPengguna)
                                    Telah Mengulas
                                @else
                                    Submit Ulasan
